...
const EvaluateSchema = z.object({
  enough: z.boolean().describe("localContext和webContext是否足够回答问题"),
  missing: z
    .array(z.string())
    .describe("localContext和webContext是否缺少的片段"),
  reason: z.string().describe("评估理由"),
  webQuery: z
    .string()
    .describe("localContext和webContext不足以回答问题时，需要网络查询的问题"),
});
...
async function bochaWebSearch(query, count = 8) {
  ...
  if (!response.ok) {
    throw new Error(`搜索 API 失败: ${response.status}`);
  }
...
const generateNode = async (state) => {
  ...
  let context = localContext
    .map(
      (item, i) =>
        `【本地检索片段${i + 1}】. 章节：第${item.chapter_num}章，内容：${item.content}`,
    )
    .join("\n\n--------\n\n");
...
async function retrieveNode(state) {
  ...
  const mergedDocuments = mergeUnique(localContext, newDocuments);
...
async function grade(state) {
  const { localContext, webContext, generation, strategy } = state;
...
【引用片段：】${localContext?.map?.((doc) => doc.content).join("\n\n--------\n\n")}
