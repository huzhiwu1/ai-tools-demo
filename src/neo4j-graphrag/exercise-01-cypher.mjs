...
// 3. 运行：node src/neo4j-graphrag/exercise-01-cypher.mjs
//
// 不会写的时候可以查看 03-query-graph.mjs 作为参考
// ============================================
...
async function main() {
    ...
    await runCypher(
        '练习 2：查询宫保鸡丁',
        // TODO：补全下面的 Cypher
        'MATCH (p:Product {name: "宫保鸡丁"}) RETURN p.name AS name'
    )
    ...
    await runCypher(
        '练习 3：番茄炒蛋的食材',
        // TODO：补全下面的 Cypher
        'MATCH (p:Product {name: "番茄炒蛋"})-[:包含]->(i:Ingredient) RETURN p.name AS product, i.name AS ingredient'
    )
    ...
    await runCypher(
        '练习 5：宫保鸡丁食材的做法',
        // TODO：补全下面的 Cypher
        'MATCH (p:Product {name: "宫保鸡丁"})-[:包含]->(i:Ingredient)-[:做法]->(m:Method) RETURN p.name AS product, i.name AS ingredient, m.name AS method'
    )
    ...
    await runCypher(
        '练习 6：每种类型的菜品数量',
        // TODO：补全下面的 Cypher
        'MATCH (p:Product)-[:属于]->(t:Type) RETURN t.name AS type, count(p) AS count, collect(p.name) AS dishes'
    )
    ...
    await runCypher(
        '练习 7：推荐给李四的菜及其类型',
        // TODO：补全下面的 Cypher
        'MATCH (p:Product)-[:推荐]->(pe:People {name: "李四"}), (p)-[:属于]->(t:Type) RETURN p.name AS product, pe.name AS person, t.name AS type'
    )
    ...
    await runCypher(
        '练习 8：炖的食材及包含它们的菜',
        // TODO：补全下面的 Cypher
        'MATCH (i:Ingredient)-[:做法]->(m:Method {name: "炖"})<-[:包含]-(p:Product) RETURN i.name AS ingredient, m.name AS method, p.name AS product'
    )
