/**
 * Exercise 06: 子 Agent Middleware — 多 Agent 协作
 *
 * 学习目标：
 * 1. 掌握 createSubAgentMiddleware 的多 Agent 委派机制
 * 2. 理解主 Agent（调度）和子 Agent（执行）的分工
 * 3. 掌握子 Agent 的定义方式（name, description, systemPrompt, tools）
 * 4. 理解 generalPurposeAgent 参数的作用
 *
 * 运行方式：pnpm ex6
 */

import "dotenv/config";
import { z } from "zod";
import { ChatOpenAI } from "@langchain/openai";
import { createAgent, HumanMessage, tool } from "langchain";
import { createSubAgentMiddleware } from "deepagents";

// ============================================
// 第一步：定义子 Agent 的工具
// ============================================

/**
 * [calc 工具]
 *
 * 职责：计算两个数的加减乘除
 *
 * 关键细节：
 * - 使用 zod 定义参数 schema，每个字段都有 .describe()
 * - 返回 JSON 字符串，包含算式和结果
 * - 处理除数为 0 的边界情况
 */
const calc = tool(
  ({ a, b, op }) => {
    const ops = {
      add: a + b,
      subtract: a - b,
      multiply: a * b,
      divide: b === 0 ? NaN : a / b,
    };
    const result = ops[op];
    if (Number.isNaN(result)) {
      return JSON.stringify({ error: "除数不能为 0" });
    }
    const symbols = { add: "+", subtract: "-", multiply: "×", divide: "÷" };
    return JSON.stringify({
      expression: `${a} ${symbols[op]} ${b}`,
      result,
    });
  },
  {
    name: "calc",
    description: "计算两个数的加减乘除",
    schema: z.object({
      a: z.number().describe("左操作数"),
      b: z.number().describe("右操作数"),
      op: z
        .enum(["add", "subtract", "multiply", "divide"])
        .describe("运算类型"),
    }),
  },
);

/**
 * [divide_evenly 工具]
 *
 * 职责：把总数平均分成若干份
 *
 * 关键细节：
 * - 处理不能整除的情况（给出说明）
 * - .nonnegative() 和 .int().positive() 约束输入
 */
const divideEvenly = tool(
  ({ total, parts }) => {
    if (parts <= 0) {
      return JSON.stringify({ error: "份数须大于 0" });
    }
    const each = total / parts;
    const exact = Number.isInteger(each);
    return JSON.stringify({
      total,
      parts,
      each,
      exact,
      note: exact
        ? `每人 ${each}（整除）`
        : `每人 ${each}（不能整除，应用题可说明余数）`,
    });
  },
  {
    name: "divide_evenly",
    description: "把总数平均分成若干份，求每份多少",
    schema: z.object({
      total: z.number().nonnegative().describe("总数"),
      parts: z.number().int().positive().describe("分成几份"),
    }),
  },
);

/**
 * [make_similar_problem 工具]
 *
 * 职责：按模板生成同类练习题（只改数字）
 *
 * 关键细节：
 * - template 参数选择题目模板
 * - seed 参数用于变换数字（同 seed 同结果）
 * - 支持三种模板：divide_then_add、share_candy、group_buy
 */
const makeSimilarProblem = tool(
  ({ template, seed }) => {
    const n = (seed % 7) + 3;
    const problems = {
      divide_then_add: {
        stem: `小红有 ${n * 6} 张贴纸，平均分给 ${n} 个小组，又买了 2 包每包 ${n + 2} 张的。每个小组现在一共有多少张？`,
        hint: "先平均分，再加上后来买的，注意单位是「每个小组」",
      },
      share_candy: {
        stem: `小刚有 ${n * 4} 块糖，要分给 ${n} 位同学，妈妈又买了 3 袋每袋 ${n} 块的。每位同学现在能分到多少块？`,
        hint: "与分糖题类似：先平分，再加上新增",
      },
      group_buy: {
        stem: `班里有 ${n} 个小组，每组先分到 ${n * 5} 支铅笔，老师又补了 2 盒每盒 ${n + 1} 支。每个小组现在有多少支？`,
        hint: "先算每组原有，再加上后来补的",
      },
    };
    const picked = problems[template] ?? problems.share_candy;
    return JSON.stringify({ template, ...picked });
  },
  {
    name: "make_similar_problem",
    description:
      "生成一道同类应用题。template: divide_then_add | share_candy | group_buy",
    schema: z.object({
      template: z
        .enum(["divide_then_add", "share_candy", "group_buy"])
        .describe("题目模板"),
      seed: z.number().int().describe("随机种子，用于变换数字"),
    }),
  },
);

// ============================================
// 第二步：定义子 Agent
// ============================================

/**
 * 子 Agent 定义说明
 *
 * 每个子 Agent 包含 4 个字段：
 * - name: 子 Agent 名称（主 Agent 用 task 工具委派时使用）
 * - description: 子 Agent 的能力描述（主 Agent 据此选择委派给谁）
 * - systemPrompt: 子 Agent 的系统提示词
 * - tools: 子 Agent 可用的工具列表
 *
 * 关键洞察：
 * - description 决定了主 Agent 会不会选对子 Agent！
 *   description 越清晰，主 Agent 的委派越准确
 * - 每个子 Agent 有独立的 systemPrompt，互不干扰
 * - 子 Agent 只能用自己 tools 列表中的工具
 */
const subagents = [
  {
    name: "math-solver",
    description:
      "解小学应用题：用 calc、divide_evenly 列式计算，给出最终答案与算式。有具体数字时先用此 Agent。",
    systemPrompt: [
      "你是解题子 Agent。",
      "必须用 calc、divide_evenly 完成计算，不要心算。",
      "输出：题目理解、分步算式、最终答案（带单位「块/人」等）。",
    ].join("\n"),
    tools: [calc, divideEvenly],
  },
  {
    name: "kid-tutor",
    description:
      "把 math-solver 的解法讲给家长听，方便辅导孩子。description 里会有完整解题过程。",
    systemPrompt: [
      "你是辅导讲解子 Agent，面向小学生家长。",
      "根据 description 中的解题过程，用短句、比喻或分步提问方式讲解（不要堆公式）。",
      "说明：先想什么、再算什么、怎么检查答案。不使用工具。",
    ].join("\n"),
    tools: [],
  },
  {
    name: "practice-maker",
    description:
      "出 2 道同类练习题。用 make_similar_problem 生成题干，可换不同 template 或 seed。",
    systemPrompt: [
      "你是出题子 Agent。",
      "调用 make_similar_problem 至少 2 次（不同 template 或不同 seed），",
      "每道题给出：题干、解题提示（一句话）。",
    ].join("\n"),
    tools: [makeSimilarProblem],
  },
];

// TODO: 添加一个"检查答案"的子 Agent
// 提示：
// {
//   name: "answer-checker",
//   description: "检查 math-solver 给出的答案是否正确，验证计算步骤。",
//   systemPrompt: "你是答案检查子 Agent。根据 description 中的解题过程，逐步验证每个计算步骤是否正确...",
//   tools: [calc],  // 可以用 calc 验证计算
// },

// ============================================
// 第三步：创建主 Agent
// ============================================

const model = new ChatOpenAI({
  model: process.env.MODEL_NAME,
  apiKey: process.env.OPENAI_API_KEY,
  configuration: { baseURL: process.env.OPENAI_BASE_URL },
  temperature: 0,
  streaming: true,
});

/**
 * [createSubAgentMiddleware]
 *
 * 职责：为主 Agent 提供"委派子 Agent"的能力
 *
 * 关键参数：
 * - defaultModel: 子 Agent 使用的默认模型
 * - subagents: 子 Agent 定义列表
 * - generalPurposeAgent: false 表示主 Agent 只能委派，不能自己回答
 *
 * 工作流程：
 * 1. 用户发送消息
 * 2. 主 Agent 分析消息，决定委派给哪个子 Agent
 * 3. 主 Agent 调用 task 工具（由 Middleware 自动注入）
 * 4. 子 Agent 执行任务，返回结果
 * 5. 主 Agent 根据结果，继续委派下一个子 Agent 或汇总
 *
 * 关键洞察：
 * - generalPurposeAgent: false → 主 Agent 只做调度，不做执行
 *   这就像项目经理不写代码，但知道把需求分配给谁
 * - generalPurposeAgent: true → 主 Agent 既可以委派，也可以自己回答
 *   适合简单场景，但不保证职责分离
 */
const agent = createAgent({
  model,
  tools: [], // 主 Agent 自己没有工具！
  systemPrompt: [
    "你是小学数学辅导主 Agent，通过 task 委派子 Agent，自己不解题、不讲题、不出题。",
    "按顺序：① math-solver ② kid-tutor（把 solver 完整过程写进 description）③ practice-maker。",
    "最后向家长汇总：答案、辅导要点、两道练习题。中文。",
  ].join("\n"),
  middleware: [
    createSubAgentMiddleware({
      defaultModel: model,
      subagents,
      generalPurposeAgent: false,
      // TODO: 如果添加了 answer-checker 子 Agent，
      // 可以在 systemPrompt 中添加 ④ answer-checker
    }),
  ],
});

// ============================================
// 第四步：运行 Agent（流式输出）
// ============================================

const prompt = [
  "孩子遇到这道题：",
  "「小明有 24 块糖，平均分给 6 个同学；",
  "妈妈又买了 3 包糖，每包 5 块。每个同学现在一共有多少块？」",
  "请先 math-solver 解题，再 kid-tutor 教家长怎么讲，",
  "最后 practice-maker 出 2 道类似练习题，并汇总给我。",
].join("");

/**
 * 流式输出处理
 *
 * 职责：实时输出 Agent 的中间过程
 *
 * 关键细节：
 * - streamEvents 返回多种事件：on_chat_model_stream（文本流）、on_tool_start（工具调用）
 * - chunk.content 可能是字符串或数组，需要统一处理
 * - 流式输出让用户无需等待完整结果，体验更好
 */
function chunkText(chunk) {
  if (!chunk?.content) return "";
  if (typeof chunk.content === "string") return chunk.content;
  if (Array.isArray(chunk.content)) {
    return chunk.content
      .map((p) => (typeof p === "string" ? p : (p?.text ?? "")))
      .join("");
  }
  return "";
}

console.log("场景: 小学应用题辅导（解题 → 讲题 → 出题）");
console.log("子 Agent:");
console.log("  math-solver     → calc, divide_evenly");
console.log("  kid-tutor       → （讲解，无工具）");
console.log("  practice-maker  → make_similar_problem");
console.log();

console.log("用户:", prompt, "\n");
console.log("--- 流式输出 ---\n");

const stream = await agent.streamEvents(
  { messages: [new HumanMessage(prompt)] },
  { recursionLimit: 60 },
);

try {
  for await (const event of stream) {
    // 文本流：实时输出 LLM 生成的文字
    if (event.event === "on_chat_model_stream") {
      const t = chunkText(event.data?.chunk);
      if (t) process.stdout.write(t);
    }
    // 工具调用开始：打印工具名称
    if (event.event === "on_tool_start") {
      const name = event.name?.split("/").pop() ?? event.name;
      process.stdout.write(`\n\n→ ${name}\n\n`);
    }
  }
} catch (e) {
  console.error("\n\n[错误]", e.cause?.message ?? e.message);
  throw e;
}

console.log("\n");

/**
 * 预期输出：
 *
 * 场景: 小学应用题辅导（解题 → 讲题 → 出题）
 * 子 Agent:
 *   math-solver     → calc, divide_evenly
 *   kid-tutor       → （讲解，无工具）
 *   practice-maker  → make_similar_problem
 *
 * 用户: 孩子遇到这道题...
 *
 * --- 流式输出 ---
 *
 * → task (委派给 math-solver)
 *
 * → divide_evenly
 * → calc
 * (解题过程...)
 *
 * → task (委派给 kid-tutor)
 * (讲解过程...)
 *
 * → task (委派给 practice-maker)
 *
 * → make_similar_problem
 * → make_similar_problem
 * (出题过程...)
 *
 * (最终汇总...)
 *
 * 知识扩展：
 *
 * Q: 主 Agent 怎么知道什么时候委派给谁？
 * A: 靠子 Agent 的 description！当主 Agent 分析用户请求后，
 *    它会根据 description 判断哪个子 Agent 最适合当前任务。
 *    这就是为什么 description 写得清晰很重要。
 *
 * Q: 为什么主 Agent 的 tools 是空数组？
 * A: 因为主 Agent 只做调度，不做执行。它的"工具"是 task（由 Middleware 注入），
 *    用来委派任务给子 Agent。这种设计确保了职责分离。
 *
 * Q: generalPurposeAgent: false 有什么好处？
 * A: 1. 防止主 Agent "越俎代庖"自己回答（可能不够专业）
 *    2. 确保每个子任务都由专门的子 Agent 处理
 *    3. 便于追踪每个步骤的执行者
 *    生产环境推荐 false，开发调试时可以用 true 灵活测试。
 */
