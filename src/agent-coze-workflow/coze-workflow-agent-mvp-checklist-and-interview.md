...
### 7.7 Coze 平台接入（2026-08-12 实测沉淀，本项目独有素材）
- **乐观锁/版本控制**：为什么每次 save 前必须重新 canvas 拿 `submit_commit_id`？（save 成功会推进 commit，用旧 commit 会被拒——这就是“乐观锁”思想的真实案例）
- **编辑锁设计**：为什么首次 save 前必须 `edit_lock(acquire)`？（平台用“获取编辑锁→建立会话→提交”保证单写者；类比 git checkout / 数据库行锁）
- **如何逆向后端 API**：没有文档怎么搞？（浏览器 DevTools 抓 cURL → 对比 JS bundle 里的接口定义 → 逐个参数试错 → 从报错信息反推类型约束，如 space_id 字符串 vs flow_mode 数字）
- **“不是最新副本”报错的排查思路**：先排除 schema 格式（用用户成功样本对照）→ 再排除 header（复制完整浏览器请求）→ 最后定位到会话状态（edit_lock）——讲清这个排查过程本身就是加分项
- **平台自有格式适配**：公开 Coze 的节点格式 ≠ 私有平台格式（id/type 是字符串数字、引用用 blockID、edges 只是展示）——说明“对接外部系统时，schema 适配层是刚需”
- **凭证安全**：cookie 短期凭证 vs PAT 长期凭证；为什么 PAT 不被 /api 接口接受时要用 cookie；为什么 key 只进 .env 不提交
