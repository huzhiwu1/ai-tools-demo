...
const tools = [readFileTool, writeFileTool, execCommandTool, listDirectoryTool];
...
回复要简洁，只说做了什么`),
  ...
  for (let i = 0; i < maxIteractions; i++) {
    console.log(chalk.green(`正在等待ai思考，第${i + 1}轮`));
...
2. 使用 pnpm run dev 启动服务器(执行这一步之前，确保项目目录下有 node_modules 目录)
`;
